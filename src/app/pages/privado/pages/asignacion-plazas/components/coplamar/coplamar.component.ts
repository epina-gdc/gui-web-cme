...
export class CoplamarComponent extends GeneralComponent implements OnInit {
  ...
  tipoAsignacion = TipoAsignacion.PlazaCoplamar;
  ...
  getZona():void{
    ...
    this.asignacionPlazaService.getZonaByOoad(Regimen.Complamar, idEspecialidad, idOoad).subscribe({
      next: (result) => {
        if (result.exito && Array.isArray(result.respuesta) && result.respuesta.length > 0) {
  ...
  getTurno():void{
    ...
    this.asignacionPlazaService.getTurnoByEspecialidad(Regimen.Complamar, idEspecialidad).subscribe({
      next: (result) => {
        if (result.exito && Array.isArray(result.respuesta) && result.respuesta.length > 0) {
  ...
  getMarcaOcupacion(): void {
    ...
    this.asignacionPlazaService.getMarcaOcupacionByEspecialidad(Regimen.Complamar, idEspecialidad).subscribe({
      next: (result) => {
  ...
  getHorario(): void {
    ...
    this.asignacionPlazaService.getHorarioByTurno(Regimen.Complamar, idEspecialidad, idTurno).subscribe({
      next: (result) => {
